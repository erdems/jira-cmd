...
define([
  ...
  'jsonpath',
  '../../lib/auth'
], function (request, Table, config, async, alasql, jsonpath, Auth) {
  ...
  var ls = {
    ...
    callIssueApi: function (options, cb) {
      ...
      async.doWhilst(function (callback) {
        ...
        var endpoint = config.auth.url + (config.auth.url.endsWith('/') ? '' : '/') + 'rest/api/3/search/jql';
          ...
          .end(function (res) {
            if (!res.ok) {
              ...
              if (res.body && res.body.errorMessages) {
                return callback(res.body.errorMessages.join('\n'));
              }
              return callback((res.error || 'Request failed').toString());
            }
    ...
    getIssues: function (options, cb) {
      ...
      this.callIssueApi(options, function (err, issues) {
        ...
        if (options && options.json && !options.custom_sql) {
        ...
        for (i = 0; i < that.issues.length; i += 1) {
          ...
          Object.values(that.getListIssuesColumns()).forEach(function (valueSpec) {
            ...
            //if not doing this then
            //when a cell is empty, later cells were shifted to empty cells
            if(!value){
              value="";
            }
            row.push(value);
    ...
    showAll: function (options, cb) {
      ...
      this.query = 'assignee=currentUser()' + this.type + ' AND status in ("' + this.getAvailableStatuses() + '")' + ' ORDER BY priority DESC, key ASC';
      return this.getIssues(options, cb);
    },

    showInProgress: function (cb) {
      this.query = 'assignee=currentUser()' + ' AND status in ("In Progress")' + ' ORDER BY priority DESC, key ASC';
      return this.getIssues(cb);
    },

    showByProject: function (options, cb) {
      this.type = (options && options.type) ? ' AND type=' + options.type : '';
      this.query = 'assignee=currentUser()' + this.type + ' AND project=' + options.project + ' AND status in ("' + this.getAvailableStatuses() + '")' + ' ORDER BY priority DESC, key ASC';
      return this.getIssues(options, cb);
    },

    search: function (query, cb) {
      this.query = 'summary ~ "' + query + '"' + ' OR description ~ "' + query + '"' + ' OR comment ~ "' + query + '"' + ' ORDER BY priority DESC, key ASC';
      return this.getIssues(cb);
    },
