  ...
  var create = {
    ...
    getMeta: function (key, callback) {
      ...
      request
        ...
        .end(function (err, res) {
          if (err) {
            console.log('Error in getMeta request:', err);
            return callback(err);
          }
          if (!res.ok) {
            console.log('Error in getMeta request - response not ok:', res.status, res.text);
            console.log((res.body.errorMessages || [res.error]).join('\n'));
            return callback((res.body.errorMessages || [res.error]).join('\n'));
          }
          console.log(res.body)
          async.eachLimit(res.body.issueTypes, 2, function iterator(eachValue, icb){
            ...
            let projectMeta = {
              issuetypes: res.body.issueTypes
            }
    ...
    getProject: function ( callback) {
      ...
      request
        ...
        .end(function (err, res) {
          if (err) {
            console.log('Error in getProject request:', err);
            return callback(err);
          }
          if (!res.ok) {
            console.log('Error in getProject request - response not ok:', res.status, res.text);
            console.log((res.body.errorMessages || [res.error]).join('\n'));
            return callback((res.body.errorMessages || [res.error]).join('\n'));
          }
    ...
    getProjectIssueFields: function (project, issueType, callback) {
      ...
      request
        ...
        .end(function (err, res) {
          if (err) {
            console.log('Error in getProjectIssueFields request:', err);
            return callback(err);
          }
          if (!res.ok) {
            console.log('Error in getProjectIssueFields request - response not ok:', res.status, res.text);
            console.log(res)
            console.log((res.body.errorMessages || [res.error]).join('\n'));
            return callback((res.body.errorMessages || [res.error]).join('\n'));
          }
    ...
    saveIssue: function (cb) {
      ...
      request
        ...
        .end(function (err, res) {
          if (err) {
            console.log('Error in saveIssue request:', err);
            return cb(err);
          }
          if (!res.ok) {
            console.log('Error in saveIssue request - response not ok:', res.status, res.text)
    ...
    create: function(options, cb){      
      //remove parent from options as its being set to some value from commander
      // and it coincides with the parent field in the issue type
      ...
      async.waterfall([
        ...
        function(options, wcb){
          ...
          create.askIssueType(options.type, function(issueType){
            ...
            create.projectMeta.issuetypes.forEach(function(eachIssueType){
              //console.log(eachIssueType, issueType)
        ...
        function (options, wcb){
          //delete create.answers.fields.parent;
